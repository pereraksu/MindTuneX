...
def get_recommendation_type(emotion: str) -> str:
    rec_map = {
        "joy": "maintain_positive_mood",
        "love": "warm_supportive_content",
        "calm": "mindfulness_and_relaxation",
        "stress": "stress_relief_content",
        "anxiety": "calming_and_reassurance",
        "sadness": "comfort_and_support",
        "anger": "cool_down_and_reflect",
        "fatigue": "rest_and_low_energy_support",
        "fear": "grounding_and_reassurance",
        "disgust": "distraction_and_reset",
        "surprise": "light_reflection_content",
        "neutral": "general_reflection_content",
    }
...
def apply_post_rules(text: str, pred: str, confidence: float) -> str:
    ...
    # 1. Explicit neutral expressions only
    ...
    # 3. Strong stress-specific cues
    if pred in ["sadness", "anxiety", "neutral", "fear"] and any(word in text_lower for word in [
        "stressed",
        "stress",
        "deadline",
        "deadlines",
    ...
    # 4. If model predicts joy but text clearly sounds calm/relaxed
    if pred == "joy" and any(word in text_lower for word in [
        "peaceful",
        "relaxing",
        "calm",
        "serene",
    ...
    # 5. If model predicts love but sentence is actually achievement happiness
    if pred == "love" and any(word in text_lower for word in [
        "happy",
        "great",
        "amazing",
        "excited",
